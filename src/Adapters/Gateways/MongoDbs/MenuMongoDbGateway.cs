...
public class MenuMongoDbGateway : MongoGatewayBase<MenuItemMongoDb>, IMenuMongoDbGateway
{
    ...
    public async Task<IEnumerable<MenuItemMongoDb>> GetAllAsync(
        string? name,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            filters.Add(builder.Regex(e => e.Name, new BsonRegularExpression(name, "i")));
        }

        if (category is not null)
        {
            filters.Add(builder.Eq(e => e.Category, category.Value));
        }
